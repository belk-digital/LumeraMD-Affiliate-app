    ...
    try {
      await Promise.all(
        Array.from(selected).map((id) =>
          fetch(`/api/lumera-ops/affiliates/${id}`, {
            method: "PATCH",
              ...
              {initialAffiliates.length > 0 ? initialAffiliates.map(a => (
                <tr key={a.id} className="hover:bg-page-bg/30 transition group">
                  ...
                  <td className="px-6 py-4">
                    <Link href={`/lumera-ops/affiliates/${a.id}`} className="flex items-center gap-3">
                  ...
                  <td className="px-6 py-4">
                    {a.parentAffiliateId ? (
                      <Link href={`/lumera-ops/affiliates/${a.parentAffiliateId}`} className="text-primary hover:underline font-medium text-xs">
                        view
                      </Link>
                    ) : (
...
function AddAffiliateModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/lumera-ops/affiliates", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
