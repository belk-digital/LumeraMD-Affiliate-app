...
export default function ApplicationActions({
  ...
  // approved — see the review modal (onReview) and /api/lumera-ops/applications/[id]/verify-w9.
  ...
  async function act(action: "approve" | "reject") {
    ...
    setMenuOpen(false);
    try {
      const res = await fetch(`/api/lumera-ops/applications/${id}/${action}`, { method: "POST" });
      if (!res.ok) {
  ...
  if (status === "approved" && linkedAffiliateId) {
    return (
      <div className="flex justify-end">
        <Link
          href={`/lumera-ops/affiliates/${linkedAffiliateId}`}
          className="rounded-lg border border-line px-3.5 py-2 text-xs font-medium text-ink/70 transition hover:bg-page-bg"
        >
          View
        </Link>
