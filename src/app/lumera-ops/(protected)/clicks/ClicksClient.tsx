  ...
  return (
    <tr className="hover:bg-page-bg/30 transition group">
      ...
      <td className="px-6 py-4">
        <Link href={`/lumera-ops/affiliates/${click.affiliateId}`} className="font-medium text-ink hover:text-primary transition">
...
function ActionMenu({ click }: { click: any }) {
  ...
  const toggleSuspicious = async () => {
    setIsUpdating(true);
    try {
      const res = await fetch(`/api/lumera-ops/clicks/${click.id}`, {
        method: "PATCH",
    ...
    <div className="relative inline-block text-left" onClick={(e) => e.stopPropagation()}>
      ...
      {isOpen && (
        ...
        >
          <div className="py-1">
            <Link
              href={`/lumera-ops/affiliates/${click.affiliateId}`}
