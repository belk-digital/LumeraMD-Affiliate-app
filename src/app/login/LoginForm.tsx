...
export default function LoginForm({ initialError }: { initialError?: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSubmitting(true);
    await fetch("/api/affiliates/auth/request", {
      method: "POST",
    ...
    return (
      <div className="animate-fade-up rounded-2xl border border-line bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-ink/60">
          If <span className="font-medium text-ink">{email}</span> is an approved affiliate
          account, we&apos;ve sent a login link. It expires in 15 minutes.
        </p>
      </div>
    );
  }
