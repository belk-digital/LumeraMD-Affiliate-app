...
function ReviewModal({
  ...
  async function handleVerify() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/applications/${a.id}/verify-w9`, { method: "POST" });
  ...
  async function handleApprove() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/applications/${a.id}/approve`, { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        ...
        <div className="max-h-[70vh] space-y-5 overflow-y-auto p-6 text-sm">
          ...
          <div className="rounded-xl border border-line p-4">
            ...
            {needsW9 ? (
              <>
                <a
                  href={`/api/admin/applications/${a.id}/w9`}
                  target="_blank"
                  rel="noopener noreferrer"
