...
function ConversionRow({ conversion }: { conversion: any }) {
  ...
  const handleReverse = async () => {
    setIsReversing(true);
    try {
      const res = await fetch(`/api/admin/conversions/${conversion.id}/reverse`, {
        method: "POST",
      });
      if (res.ok) {
    ...
    <>
      <tr className="hover:bg-page-bg/30 transition group">
        <td className="px-6 py-4 font-semibold text-primary hover:underline">
          <Link href={conversion.shopifyOrderId ? `https://admin.shopify.com/store/lumeramd/orders/${conversion.shopifyOrderId}` : "#"} target="_blank" className="flex items-center gap-1.5">
